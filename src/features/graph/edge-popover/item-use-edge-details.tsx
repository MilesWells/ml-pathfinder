...
export function ItemUseEdgeDetails({ edge: { from, to, item } }: ItemUseEdgeDetailsProps) {
	...
	if (item === 'Return to New Leaf City Scroll') return <Text fs="italic">Only works within Victoria Island</Text>;

	if (item === 'Fruit Milk' || item === 'Strawberry Milk') return <Text fs="italic">Only works within Zipangu</Text>;
